...
CAMERA_STATUS_NOT_SUPPORTED = -4   # 이 기능은 지원되지 않음
CAMERA_STATUS_NOT_INITIALIZED = -5   # 초기화가 완료되지 않음
CAMERA_STATUS_PARAMETER_INVALID = -6   # 매개변수가 잘못됨
CAMERA_STATUS_PARAMETER_OUT_OF_BOUND = -7   # 매개변수가 범위를 벗어남
CAMERA_STATUS_UNENABLED = -8   # 활성화되지 않음
CAMERA_STATUS_USER_CANCEL = -9   # 사용자가 취소함, 예: ROI 패널에서 취소를 클릭하면 반환됨
CAMERA_STATUS_PATH_NOT_FOUND = -10  # 레지스트리에서 해당 경로를 찾을 수 없음
CAMERA_STATUS_SIZE_DISMATCH = -11  # 이미지 데이터 길이와 정의된 크기가 일치하지 않음
...
CAMERA_STATUS_NO_DEVICE_FOUND = -16  # 장치를 찾을 수 없음
CAMERA_STATUS_NO_LOGIC_DEVICE_FOUND = -17  # 논리 장치를 찾을 수 없음
CAMERA_STATUS_DEVICE_IS_OPENED = -18  # 장치가 이미 열려 있음
CAMERA_STATUS_DEVICE_IS_CLOSED = -19  # 장치가 이미 닫혀 있음
CAMERA_STATUS_DEVICE_VEDIO_CLOSED = -20  # 장치 비디오가 열려 있지 않습니다. 비디오 관련 함수를 호출할 때 카메라 비디오가 열려 있지 않으면이 오류가 반환됩니다.
CAMERA_STATUS_NO_MEMORY = -21  # 충분한 시스템 메모리가 없음
CAMERA_STATUS_FILE_CREATE_FAILED = -22  # 파일 생성 실패
CAMERA_STATUS_FILE_INVALID = -23  # 파일 형식이 잘못됨
CAMERA_STATUS_WRITE_PROTECTED = -24  # 쓰기 보호, 쓸 수 없음
CAMERA_STATUS_GRAB_FAILED = -25  # 데이터 수집 실패
CAMERA_STATUS_LOST_DATA = -26  # 데이터 손실, 불완전
CAMERA_STATUS_EOF_ERROR = -27  # 프레임 종료 기호를 수신하지 못함
CAMERA_STATUS_BUSY = -28  # 작업 진행 중 (이전 작업이 아직 진행 중인 경우)
CAMERA_STATUS_WAIT = -29  # 대기 필요 (작업 조건이 충족되지 않음), 다시 시도할 수 있음
CAMERA_STATUS_IN_PROCESS = -30  # 진행 중이며 이미 작업되었음
CAMERA_STATUS_IIC_ERROR = -31  # IIC 전송 오류
CAMERA_STATUS_SPI_ERROR = -32  # SPI 전송 오류
CAMERA_STATUS_USB_CONTROL_ERROR = -33  # USB 제어 전송 오류
CAMERA_STATUS_USB_BULK_ERROR = -34  # USB BULK 전송 오류
CAMERA_STATUS_SOCKET_INIT_ERROR = -35  # 네트워크 전송 스위트 초기화 실패
CAMERA_STATUS_GIGE_FILTER_INIT_ERROR = -36  # 네트워크 카메라 커널 필터 드라이버 초기화 실패, 드라이버가 올바르게 설치되었는지 또는 다시 설치했는지 확인하십시오.
CAMERA_STATUS_NET_SEND_ERROR = -37  # 네트워크 데이터 전송 오류
CAMERA_STATUS_DEVICE_LOST = -38  # 네트워크 카메라와의 연결 손실, 하트 비트 검사 시간 초과
CAMERA_STATUS_DATA_RECV_LESS = -39  # 요청보다 적은 바이트를 수신
CAMERA_STATUS_FUNCTION_LOAD_FAILED = -40  # 파일에서 프로그램을로드하지 못했습니다.
CAMERA_STATUS_CRITICAL_FILE_LOST = -41  # 프로그램 실행에 필요한 파일이 손실되었습니다.
CAMERA_STATUS_SENSOR_ID_DISMATCH = -42  # 펌웨어와 프로그램이 일치하지 않습니다. 잘못된 펌웨어를 다운로드한 이유입니다.
CAMERA_STATUS_OUT_OF_RANGE = -43  # 유효한 범위를 벗어남
CAMERA_STATUS_REGISTRY_ERROR = -44  # 설치 프로그램 레지스트리 오류. 프로그램을 다시 설치하거나 설치 디렉토리의 Setup/Installer.exe를 실행하십시오.
CAMERA_STATUS_ACCESS_DENY = -45  # 액세스 거부. 특정 카메라가 다른 프로그램에 의해 사용 중일 때 해당 카메라에 액세스하려고하면이 상태가 반환됩니다. (카메라는 여러 프로그램에서 동시에 액세스 할 수 없음)

# AIA의 표준 호환 오류 코드
CAMERA_AIA_PACKET_RESEND = 0x0100  # 이 프레임을 다시 전송해야 함
CAMERA_AIA_NOT_IMPLEMENTED = 0x8001  # 장치에서 지원하지 않는 명령
CAMERA_AIA_INVALID_PARAMETER = 0x8002  # 명령 매개변수가 잘못됨
CAMERA_AIA_INVALID_ADDRESS = 0x8003  # 액세스할 수 없는 주소
CAMERA_AIA_WRITE_PROTECT = 0x8004  # 액세스할 수 없는 객체 쓰기
CAMERA_AIA_BAD_ALIGNMENT = 0x8005  # 액세스한 주소가 요구 사항에 맞게 정렬되지 않음
CAMERA_AIA_ACCESS_DENIED = 0x8006  # 액세스 권한이 없음
CAMERA_AIA_BUSY = 0x8007  # 명령이 처리 중임
CAMERA_AIA_DEPRECATED = 0x8008  # 0x8008-0x0800B 및 0x800F: 해당 명령이 폐기됨
CAMERA_AIA_PACKET_UNAVAILABLE = 0x800C  # 패킷이 유효하지 않음
CAMERA_AIA_DATA_OVERRUN = 0x800D  # 데이터 오버런, 일반적으로 받은 데이터가 필요한 것보다 많음
CAMERA_AIA_INVALID_HEADER = 0x800E  # 데이터 패킷 헤더의 일부가 프로토콜과 일치하지 않음
CAMERA_AIA_PACKET_NOT_YET_AVAILABLE = 0x8010  # 이미지 패킷 데이터가 아직 준비되지 않았음, 주로 트리거 모드에 사용되며 애플리케이션이 시간 초과됨
CAMERA_AIA_PACKET_AND_PREV_REMOVED_FROM_MEMORY = 0x8011  # 액세스해야 할 패킷이 더 이상 존재하지 않음, 주로 다시 전송 시 데이터가 버퍼에 없을 때 사용
CAMERA_AIA_PACKET_REMOVED_FROM_MEMORY = 0x8012  # CAMERA_AIA_PACKET_AND_PREV_REMOVED_FROM_MEMORY와 동일
CAMERA_AIA_NO_REF_TIME = 0x0813  # 참조 시계 원본 없음, 주로 시간 동기화 명령 실행 중에 사용
CAMERA_AIA_PACKET_TEMPORARILY_UNAVAILABLE = 0x0814  # 대역폭 문제로 현재 패킷 일부가 일시적으로 사용 불가능함, 나중에 액세스해야 함
CAMERA_AIA_OVERFLOW = 0x0815  # 디바이스 측 데이터 오버플로우, 일반적으로 큐가 가득 찬 경우
CAMERA_AIA_ACTION_LATE = 0x0816  # 명령 실행이 유효한 시간을 초과함
...
# 저장 형식 정의
FILE_JPG = 1  # JPEG 이미지 파일 형식
FILE_BMP = 2  # 비트맵 이미지 파일 형식
FILE_RAW = 4  # RAW 이미지 파일 형식
FILE_PNG = 8  # PNG 이미지 파일 형식
FILE_BMP_8BIT = 16  # 8비트 BMP 이미지 파일 형식
FILE_PNG_8BIT = 32  # 8비트 PNG 이미지 파일 형식
FILE_RAW_16BIT = 64  # 16비트 RAW 이미지 파일 형식

# 트리거 신호
EXT_TRIG_LEADING_EDGE = 0  # 상승 에지 트리거
EXT_TRIG_TRAILING_EDGE = 1  # 하강 에지 트리거
EXT_TRIG_HIGH_LEVEL = 2  # 고수준 레벨 트리거
EXT_TRIG_LOW_LEVEL = 3  # 저수준 레벨 트리거
EXT_TRIG_DOUBLE_EDGE = 4  # 이중 에지 트리거

# IO 모드
IOMODE_TRIG_INPUT = 0  # 트리거 입력 모드
IOMODE_STROBE_OUTPUT = 1  # 스트로브 출력 모드
IOMODE_GP_INPUT = 2  # 일반 목적 입력 모드
IOMODE_GP_OUTPUT = 3  # 일반 목적 출력 모드
IOMODE_PWM_OUTPUT = 4  # PWM 출력 모드
...
# 카메라 장치 정보, 읽기 전용 정보로 수정하지 마십시오.
class tSdkCameraDevInfo(MvStructure):
	_fields_ = [("acProductSeries", c_char * 32),  # 제품 시리즈
                ("acProductName", c_char * 32),  # 제품 이름
                ("acFriendlyName", c_char * 32),  # 제품 별명
                ("acLinkName", c_char * 32),  # 내부 커널 심볼 연결 이름, 내부 사용
                ("acDriverVersion", c_char * 32),  # 드라이버 버전
                ("acSensorType", c_char * 32),  # 센서 유형
                ("acPortType", c_char * 32),  # 인터페이스 유형
                ("acSn", c_char * 32),  # 제품 고유 일련 번호
				("uInstance", c_uint)]  # 이 모델 카메라의 이 컴퓨터에서의 인스턴스 인덱스 번호, 동일 모델 다중 카메라 구분에 사용
