...
def camera_stream():
    ...
    def perform_object_detection(frame):
        ...
        # 색상 경계에 따른 이미지 필터링
        kernel_size = 9
        blurred_frame = cv2.GaussianBlur(frame.copy(), (kernel_size, kernel_size), 0)
        ...
        def unit_contour(i,j):
            ...
            if sum(cv2.contourArea(contour) for contour in cell_contours) > 1:
                ...
                if num_contours != 1 and  contour_area > cell_width * cell_height * threshold.get()*0.001:
        ...
        if current_button == "10x10" or "5x10" or "S_10x10":
            unit_contour_2()
    ...
    threshold_values = [46.5,35.1,51.5,13.2]
